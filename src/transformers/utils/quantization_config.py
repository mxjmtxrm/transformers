...
@dataclass
class TGIQuantizationConfig(QuantizationConfigMixin):
    """
    This is a wrapper class about all possible attributes and features that you can play with a model that has been
    loaded using `bitsandbytes`.
    ...
    Args:
        load_in_tgi_8bit (`bool`, *optional*, defaults to `False`):
            This flag is used to enable 8-bit quantization with LLM.int8().
        load_in_tgi_4bit (`bool`, *optional*, defaults to `False`):
            This flag is used to enable 4-bit quantization by replacing the Linear layers with FP4/NF4 layers from
            `bitsandbytes`.
        llm_int8_threshold (`float`, *optional*, defaults to 6.0):
            This corresponds to the outlier threshold for outlier detection as described in `LLM.int8() : 8-bit Matrix
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.quant_method = QuantizationMethod.TGI

        if load_in_tgi_4bit and load_in_tgi_8bit:
            raise ValueError("load_in_tgi_4bit and load_in_tgi_8bit are both True, but only one can be used at the same time")

        self._load_in_tgi_8bit = load_in_tgi_8bit
    ...
    @property
    def load_in_tgi_4bit(self):
        return self._load_in_tgi_4bit

    @load_in_tgi_4bit.setter
    def load_in_tgi_4bit(self, value: bool):
        if not isinstance(value, bool):
            raise ValueError("load_in_tgi_4bit must be a boolean")

        if self.load_in_tgi_8bit and value:
            raise ValueError("load_in_tgi_4bit and load_in_tgi_8bit are both True, but only one can be used at the same time")
        self._load_in_tgi_4bit = value

    @property
    def load_in_tgi_8bit(self):
        return self._load_in_tgi_8bit

    @load_in_tgi_8bit.setter
    def load_in_tgi_8bit(self, value: bool):
        if not isinstance(value, bool):
            raise ValueError("load_in_tgi_8bit must be a boolean")

        if self.load_in_tgi_4bit and value:
            raise ValueError("load_in_tgi_4bit and load_in_tgi_8bit are both True, but only one can be used at the same time")
        self._load_in_tgi_8bit = value

    def post_init(self):
        r"""
        Safety checker that arguments are correct - also replaces some NoneType arguments with their default values.
        """
        if not isinstance(self.load_in_tgi_4bit, bool):
            raise ValueError("load_in_tgi_4bit must be a boolean")
    ...
    def is_quantizable(self):
        ...
        Returns `True` if the model is quantizable, `False` otherwise.
        """
        return self.load_in_tgi_8bit or self.load_in_tgi_4bit
    ...
    def quantization_method(self):
        ...
        `None`.
        """
        if self.load_in_tgi_8bit:
            return "tgi_int8"
        elif self.load_in_tgi_4bit:
            return "tgi_int4"
        else:
            return None
    ...
    def to_diff_dict(self) -> Dict[str, Any]:
        ...
        # get the default config dict
        default_config_dict = TGIQuantizationConfig().to_dict()
