...
_SEQUENCE_RING_PARALLEL_GROUP = None
_SEQUENCE_RING_PARALLEL_WORLD_SIZE = None
_SEQUENCE_RING_PARALLEL_RANK = None

_SEQUENCE_ULYSSES_PARALLEL_GROUP = None
_SEQUENCE_ULYSSES_PARALLEL_WORLD_SIZE = None
_SEQUENCE_ULYSSES_PARALLEL_RANK = None
...
def set_seq_parallel_pg(
    ...
    print(f"{sp_ring_degree=}, {sp_ulysses_degree=}")
    assert (
    ...
    if use_ulysses_low:
        ...
        for dp_rank in range(dp_degree):
            ...
            for i in range(num_ulysses_pgs):
                ...
                if rank in ulysses_ranks:
                    ulyssess_pg = group
    global _SEQUENCE_RING_PARALLEL_GROUP
    _SEQUENCE_RING_PARALLEL_GROUP = ring_pg
    global _SEQUENCE_ULYSSES_PARALLEL_GROUP
    _SEQUENCE_ULYSSES_PARALLEL_GROUP = ulyssess_pg

def initialize_model_parallel(
    tensor_model_parallel_size: int = 1,
    pipeline_model_parallel_size: int = 1,
    sequence_parallel_size: int = 1,
    sequence_ring_parallel_size: int = 1,
    ...
    ring_degree = sequence_ring_parallel_size
    ulysse_degree = sequence_parallel_size // sequence_ring_parallel_size
...
def get_sequence_ulysses_parallel_group():
    """Get the sequence ulysses parallel group the caller rank belongs to."""
    assert _SEQUENCE_ULYSSES_PARALLEL_GROUP is not None, \
        'sequence ulysses parallel group is not initialized'
    return _SEQUENCE_ULYSSES_PARALLEL_GROUP

def get_sequence_ring_parallel_group():
    """Get the sequence ring parallel group the caller rank belongs to."""
    assert _SEQUENCE_RING_PARALLEL_GROUP is not None, \
        'sequence ring parallel group is not initialized'
    return _SEQUENCE_RING_PARALLEL_GROUP
...
def get_sequence_ulysses_parallel_world_size():
    """Return world size for the sequence ulysses parallel group."""
    global _SEQUENCE_ULYSSES_PARALLEL_WORLD_SIZE
    if _SEQUENCE_ULYSSES_PARALLEL_WORLD_SIZE is not None:
        return _SEQUENCE_ULYSSES_PARALLEL_WORLD_SIZE
    return torch.distributed.get_world_size(group=get_sequence_ulysses_parallel_group())

def get_sequence_ring_parallel_world_size():
    """Return world size for the sequence ring parallel group."""
    global _SEQUENCE_RING_PARALLEL_WORLD_SIZE
    if _SEQUENCE_RING_PARALLEL_WORLD_SIZE is not None:
        return _SEQUENCE_RING_PARALLEL_WORLD_SIZE
    return torch.distributed.get_world_size(group=get_sequence_ring_parallel_group())
